...
//   };
  ...
  const distributeCalculatedValue = async (e) => {
    ...
    try {
        ...
        // Call the `transferCalculatedBNB` method
        const tx = await contract.transferCalculatedBNB();
        await tx.wait();

        alert("BNB distributed successfully!");
    } catch (error) {
        console.error("Error distributing BNB:", error);
        alert(`Error: Send some BNB in the contract`);
    }
};
